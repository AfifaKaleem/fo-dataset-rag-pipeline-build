...
def build_document(row):
    ...
    parts = [
        ...
        f"Contact: {r['dm_email']} | Website: {r['website']}.",
...
if __name__ == "__main__":
    ...
    # Check if fo_dataset.xlsx already exists
    if not os.path.exists(EXCEL_PATH):
        # Try to find the Task #1 dataset
        possible_paths = [
            "FamilyOfficeIntelligence_PolarityIQ_250Records_task1.xlsx",
            "/FamilyOfficeIntelligence_PolarityIQ_250Records_task1.xlsx",
            os.path.expanduser("~/FamilyOfficeIntelligence_PolarityIQ_250Records_task1.xlsx"),
        ]
        ...
        if src:
            shutil.copy(src, EXCEL_PATH)
            print(f"Copied dataset from {src} to {EXCEL_PATH}")
        else:
            print(f"Dataset not found at expected locations. Using existing {EXCEL_PATH} if available.")
    else:
        print(f"Dataset already exists at {EXCEL_PATH}")
